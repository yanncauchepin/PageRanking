...
setwd(dirname(rstudioapi::getSourceEditorContext()$path)) # Set le repertoire de travail au repertoire de transi.r
...
matrixTransi=genTransition(as.matrix(as_adjacency_matrix(simplify(graph_from_edgelist(cbind(from,to))))))
write.table(as.matrix(matrixTransi), file="transi.txt", row.names=FALSE, col.names=TRUE)
